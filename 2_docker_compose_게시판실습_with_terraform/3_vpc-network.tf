...
resource "aws_vpc" "st8_ex_vpc" {
  ...
  enable_dns_hostnames = true # VPC 내에서 DNS 호스트 이름을 사용할 수 있도록 활성화: default(false)
  enable_dns_support = true # VPC에서 DNS 지원 활성화: default(true)
...
resource "aws_subnet" "st8_ex_public_subnets" {
    count = 1 # 반복: 3개의 퍼블릭 서브넷 생성 / count는 첫 라인에 작성해야 함
...
resource "aws_route_table_association" "st8_ex_public2a_RT_association" {
    count=1 # 반복: 라우팅 테이블을 3개의 퍼블릭 서브넷과 연결
...
#     route {
#         cidr_block = "0.0.0.0/0"
#         gateway_id = aws_nat_gateway.st8_ex_nat_gw.id # 위에서 생성한 NAT GW의 ID
#     }
...
#     subnet_id = aws_subnet.st8_ex_private2a_subnet.id # RT와 연결할 서브넷의 ID 
# }
