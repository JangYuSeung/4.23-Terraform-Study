...
# VPC 구성
# =====================================================
resource "aws_vpc" "st8_EKS_vpc" {
  cidr_block = "10.10.0.0/16"
  enable_dns_hostnames = true # VPC 내에서 DNS 호스트 이름 활성화: default(false)
  enable_dns_support = true # VPC에서 DNS 확인 활성화: default(true)
  
  tags = { Name = "st8_EKS_vpc" }
}

#######################################################
# Public Subnet 구성 (반복문으로 3개 생성)
# =====================================================
resource "aws_subnet" "st8_EKS_public_subnets" {
    count = 3 # 반복: 3개의 퍼블릭 서브넷 생성 / count는 첫 라인에 작성해야 함
    vpc_id = aws_vpc.st8_EKS_vpc.id # 위에서 생성한 VPC의 ID
    # 
    cidr_block = "10.10.${count.index + 1}.0/24" # 큰 따옴표 안에서 ${}는 변수나 표현식 삽입을 의미 / count.index=0,1,2 순으로 증가
    ...
    tags = { 
        Name = "st8_EKS_public${count.index + 1}_subnet",
...
# =====================================================
resource "aws_subnet" "st8_EKS_private_subnets" {
    ...
    tags = { 
        Name = "st8_EKS_private${count.index + 1}_subnet",
        # 아래 추가(4.29)
...
resource "aws_nat_gateway" "st8_ex_nat_gw" {
    ...
    subnet_id = aws_subnet.st8_EKS_public_subnets[0].id # NAT GW가 위치할 퍼블릭 서브넷의 ID
...
resource "aws_route_table_association" "st8_ex_public2a_RT_association" {
    ...
    subnet_id = aws_subnet.st8_EKS_public_subnets[count.index].id # RT와 연결할 서브넷의 ID
}
...
resource "aws_route_table_association" "st8_ex_private2a_RT_association" {
    ...
    subnet_id      = aws_subnet.st8_EKS_private_subnets[count.index].id # RT와 연결할 서브넷의 ID
}
